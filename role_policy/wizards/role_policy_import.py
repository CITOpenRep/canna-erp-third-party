...
class RolePolicyImport(models.TransientModel):
    ...
    def role_policy_import(self):
        ...
        if err_log:
            ...
            _logger.warn(
                'Role %s import time = %.3f seconds',
                role.name, import_time)
    ...
    def _read_xls(self, data, role):
        if self.sheet:
            ...
            sheets = [x[0] for x in self._selection_sheet()]
        err_log = ''
        ...
        for i, sheet_name in enumerate(sheets):
            ...
            if sheet_err_log:
                if err_log:
                    err_log += '\n\n'
                err_log += _(
                    "Errors detected while importing sheet '%s'."
                ) % sheet.name
                err_log += '\n\n' + sheet_err_log
        return err_log
    ...
    def _read_acl(self, sheet, role):
        ...
        unlink_column = (
            len(headerline) > unlink_pos
            and headerline[unlink_pos] in ['Delete Entry', 'Unlink'])
        ...
        for ri in range(1, sheet.nrows):
            ...
            line_action = False
            role_acl = role.acl_ids.filtered(
    ...
    def _read_menu(self, wb, role):
        err_log = "menu import is under construction"
        return err_log

    def _read_act_window(self, wb, role):
        err_log = "act_window import is under construction"
        return err_log

    def _read_act_server(self, wb, role):
        err_log = "act_server import is under construction"
        return err_log

    def _read_act_report(self, wb, role):
        err_log = "act_report import is under construction"
        return err_log
