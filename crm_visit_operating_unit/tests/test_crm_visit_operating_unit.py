...
class TestCrmVisitOperatingUnit(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.user1 = self._create_user('user_1', [self.grp_sale_user,],
                                       self.company, [self.ou1, self.ou_unit1])
        # Create user2
        self.user2 = self._create_user('user_2', [self.grp_sale_user,],
                                       self.company, [self.ou_unit2])

        # Create Feeling1
        self.feeling1 = self._create_crm_visit_feeling('Feeling1', self.company, self.ou_unit1)
        # Create Feeling2
        self.feeling2 = self._create_crm_visit_feeling('Feeling2', self.company, self.ou_unit2)

        # Create Reason1
        self.reason1 = self._create_crm_visit_reason('Reason1', self.company, self.ou_unit1)
        # Create Reason2
        self.reason2 = self._create_crm_visit_reason('Reason2', self.company, self.ou_unit2)
    ...
    def test_security(self):
        ...
        feelings = self.crm_visit_feeling_model.sudo(
                self.user2.id
        ).search(
                [
                    ('id', '=', self.feeling1.id),
        ...
        self.assertEqual(feelings.ids, [], 'User 2 should not have access to '
                                           'OU %s' % self.ou1.name)

        reasons = self.crm_visit_reason_model.sudo(
                self.user2.id
        ).search(
                [
                    ('id', '=', self.reason1.id),
        ...
        self.assertEqual(reasons.ids, [], 'User 2 should not have access to '
                                           'OU %s' % self.ou1.name)
